...
@section('content')
    <div id="content" class="col-sm-9">
        <h1>{{ $current_m_category->title }}</h1>
        <div class="row">
            <div class="col-sm-12">
                <p>{{ $current_m_category->description }}</p>
            </div>
        </div>
        <h3><span>Refine Search</span></h3>
        <div class="category-list-thumb row" style="margin-bottom:0px;">

            @for ($i = 0; $i < count($current_m_category->subCategories); $i++)
                    <div class="col-lg-2 col-md-2 col-sm-2 col-xs-4">
                        <a href="{{ route('sub_categories.getRelatedProducts', [$current_m_category->subCategories[$i]->id, $current_m_category->subCategories[$i]->slug]) }}"><img style="height: 100px; width: 100px;" src="{{ asset($current_m_category->subCategories[$i]->image) }}"></a>
                        <a href="{{ route('sub_categories.getRelatedProducts', [$current_m_category->subCategories[$i]->id, $current_m_category->subCategories[$i]->slug]) }}">{{ $current_m_category->subCategories[$i]->title }}</a> </div>
                @if (($i + 1) % 6 === 0)
