...
$(document).ready(function () {
    $(".text-danger").each(function () {
        ...
        var t = $("#menu").offset(), e = $(this).parent().offset(),
            o = e.left + $(this).outerWidth() - (t.left + $("#menu").outerWidth());
        o > 0 && $(this).css("margin-left", "-" + (o + 5) + "px")
    }), $("#list-view").click(function () {
            $(".products-category > .clearfix.visible-lg-block").remove(),
            $("#content .product-layout").attr("class", "product-layout product-list col-xs-12"), localStorage.setItem("display", "list"),
            $(".btn-group").find("#list-view").addClass("selected"), $(".btn-group").find("#grid-view").removeClass("selected")
    }), $("[data-toggle='tooltip']").tooltip({container: "body"}), $(document).ajaxStop(function () {
        $("[data-toggle='tooltip']").tooltip({container: "body"})
    }),
        $("#grid-view").click(function () {
            $("#content .product-layout").attr("class", "product-layout product-grid col-lg-3 col-md-3 col-sm-4 col-xs-6"), localStorage.setItem("display", "grid"),
            $(".btn-group").find("#grid-view").addClass("selected"), $(".btn-group").find("#list-view").removeClass("selected")
        }), $("[data-toggle='tooltip']").tooltip({container: "body"}), $(document).ajaxStop(function () {
        $("[data-toggle='tooltip']").tooltip({container: "body"})
    })
...
var cart = {
    add: function (t, e) {
        ...
        $.ajax({
            ...
            success: function (t) {
                $("#cart > button").button("reset"), $("#cart-total").html(t.total), "checkout/cart" == getURLVar("route") || "checkout/checkout" == getURLVar("route") ? location = "index.php?route=checkout/cart" : $("#cart > ul").load("index.php?route=common/cart/info ul li")
            }
        })
    }, remove: function (t) {
        $.ajax({
